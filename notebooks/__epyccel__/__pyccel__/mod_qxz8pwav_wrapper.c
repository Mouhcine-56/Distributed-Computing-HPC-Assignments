...
long solve_2d_nonlinearconv_pyccel(int n0_u, int n1_u, double *u, int n0_un, int n1_un, double *un, int n0_v, int n1_v, double *v, int n0_vn, int n1_vn, double *vn, long nt, double dt, double dx, double dy, double c);
...
PyObject *solve_2d_nonlinearconv_pyccel_wrapper(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O!O!O!O!ldddd", kwlist, &PyArray_Type, &u, &PyArray_Type, &un, &PyArray_Type, &v, &PyArray_Type, &vn, &nt, &dt, &dx, &dy, &c))
    {
        return NULL;
    }
    if (PyArray_NDIM(u) != 2)
    ...
    Out_0001 = solve_2d_nonlinearconv_pyccel(PyArray_DIM(u, 0), PyArray_DIM(u, 1), PyArray_DATA(u), PyArray_DIM(un, 0), PyArray_DIM(un, 1), PyArray_DATA(un), PyArray_DIM(v, 0), PyArray_DIM(v, 1), PyArray_DATA(v), PyArray_DIM(vn, 0), PyArray_DIM(vn, 1), PyArray_DATA(vn), nt, dt, dx, dy, c);
    result = Py_BuildValue("l", Out_0001);
    return result;
}
/*........................................*/

static PyMethodDef mod_qxz8pwav_methods[] = {
    {
...
static struct PyModuleDef mod_qxz8pwav_module = {
    ...
    -1,
    mod_qxz8pwav_methods
};

/*........................................*/

PyMODINIT_FUNC PyInit_mod_qxz8pwav(void)
{
    PyObject *m;

    import_array();

    m = PyModule_Create(&mod_qxz8pwav_module);
