...
function solve_2d_burger_pyccel(u, un, v, vn, nt, dt, dx, dy, nu) result &
  ...
  do while (n < nt)
    ...
    do i = 1_C_LONG_LONG, row - 1_C_LONG_LONG-1_C_LONG_LONG, &
      ...
      1_C_LONG_LONG, i)) / dy + nu * dt * ((un(j, i + 1_C_LONG_LONG) - &
      2_C_LONG_LONG * un(j, i) + un(j, i - 1_C_LONG_LONG)) / dx ** &
      2_C_LONG_LONG + (un(j + 1_C_LONG_LONG, i) - 2_C_LONG_LONG * un(j, &
      i) + un(j - 1_C_LONG_LONG, i)) / dy ** 2_C_LONG_LONG)
        v(j, i) = vn(j, i) - un(j, i) * dt * (vn(j, i) - vn(j, i - &
      1_C_LONG_LONG)) / dx - vn(j, i) * dt * (vn(j, i) - vn(j - &
      1_C_LONG_LONG, i)) / dy + nu * dt * ((vn(j, i + 1_C_LONG_LONG) - &
      2_C_LONG_LONG * vn(j, i) + vn(j, i - 1_C_LONG_LONG)) / dx ** &
      2_C_LONG_LONG + (vn(j + 1_C_LONG_LONG, i) - 2_C_LONG_LONG * vn(j, &
      i) + vn(j - 1_C_LONG_LONG, i)) / dy ** 2_C_LONG_LONG)
      end do
    end do
    n = n + 1_C_LONG_LONG
  end do
